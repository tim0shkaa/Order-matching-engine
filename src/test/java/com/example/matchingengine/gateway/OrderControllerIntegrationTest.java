...
class OrderControllerIntegrationTest {
    ...
    @Test
    @DisplayName("валидная LIMIT заявка в пустой стакан возвращает 201 и корректное тело ответа")
    void createOrder_validLimitRequest_returnsCreatedWithExpectedBody() throws Exception {
    ...
    void createOrder_missingQuantity_returnsBadRequest() throws Exception {
        mockMvc.perform(post("/api/v1/orders")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"side": "SELL", "orderType": "LIMIT", "price": 50000}
                                """))
                .andExpect(status().isBadRequest());
    }

    @Test
    @DisplayName("заявка без orderType возвращает 400")
    ...
    @Test
    @DisplayName("две встречные LIMIT заявки подряд сквозь весь стек дают сделку")
    void createOrder_twoMatchingLimitOrders_producesTradeAcrossFullStack() throws Exception {
        mockMvc.perform(post("/api/v1/orders")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
    ...
    @Test
    @DisplayName("MARKET заявка сквозь весь стек — исполняется по доступной цене, не по указанной")
    void createOrder_marketOrder_matchesAtRestingPriceNotOwnPrice() throws Exception {
        mockMvc.perform(post("/api/v1/orders")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
                        {"side": "SELL", "orderType": "LIMIT", "price": 40000, "quantity": 5}
                        """));

        mockMvc.perform(post("/api/v1/orders")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"side": "BUY", "orderType": "MARKET", "quantity": 5}
                                """))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.orderStatus").value("FILLED"))
                .andExpect(jsonPath("$.trades[0].price").value(40000));
    }

    @Test
    @DisplayName("FOK заявка без достаточной ликвидности сквозь весь стек — CANCELLED, без сделок")
    void createOrder_fokOrder_notEnoughLiquidity_cancelledAcrossFullStack() throws Exception {
        mockMvc.perform(post("/api/v1/orders")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""
                        {"side": "SELL", "orderType": "LIMIT", "price": 50000, "quantity": 2}
                        """));

        mockMvc.perform(post("/api/v1/orders")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"side": "BUY", "orderType": "FOK", "price": 50000, "quantity": 5}
                                """))
                .andExpect(status().isCreated())
